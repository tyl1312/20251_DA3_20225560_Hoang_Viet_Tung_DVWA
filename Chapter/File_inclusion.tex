...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{Figure/fi_res_low.png} 
\end{figure}

\noindent \textbf{Patching:} The patching approach for low level file inclusion is to use a whitelist approach. Rather than trying to block malicious patterns, explicitly define which files are allowed to be included:
\begin{lstlisting}
<?php
...
?>
\end{lstlisting}
\noindent This ensures that only explicitly approved files can be included, preventing any arbitrary file access. Additionally, input validation should be implemented using functions like \verb|basename()| to extract only the filename from user input, removing any directory traversal attempts.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{Figure/fi_res_med.png} 
\end{figure}

\noindent \textbf{Patching:} To prevent file inclusion at the medium level, the same patching approach as in low level is applied because the core program logic has not changed. Enforce a whitelist of allowed files so that only explicitly allowed files can ever be included.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{Figure/fi_res_high.png} 
\end{figure}

\noindent \textbf{Patching:} For the high level, the patching strategy remains unchanged to the previous levels: keep a strict whitelist of allowed files. Because the vulnerable logic only tweaks the blacklist pattern, the same whitelist fix blocks all inclusion vectors.